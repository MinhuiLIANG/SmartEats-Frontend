...
export default function EndPage({ navigation }) {
  ...
  const [cusb, setCusb] = useState('');
  const [cusc, setCusc] = useState('');
  ...
  useEffect(() => {
    if (acca != '' && expa != '' && expb != '' && expc != '' && intera != '' && usefa != '' && usefb != '' && trusta != '' && trustb != '' && eata != '' && useia != '' && useib != '' && useic != '' && quaa != '' && ada != '' && adb != '' && edul != '' && workf != '' && race != '' && hper != '' && feedback != '' && extra != '') 
    {
  ...
  }, [acca, expa, expb, expc, intera, usefa, usefb, trusta, trustb, eata, useia, useib, useic, quaa, ada, adb, edul, workf, race, hper, feedback, edut, extra]);
  ...
  const handleCusaChange = (option) => {
    console.log('Selected option:', option);
    setCusa(option);
  };

  const handleCusbChange = (option) => {
    console.log('Selected option:', option);
    setCusb(option);
  };

  const handleCuscChange = (option) => {
    console.log('Selected option:', option);
    setCusc(option);
  };
...
<       Text style={{fontSize: 16 * px, color: 'white'}}>{' '}</Text>
        ...
        <SlideScale
          question={questionnaire.cusa}
          onChange={handleCusaChange}
          containerStyle={styles.valcontainer}
        />

        <SlideScale
          question={questionnaire.cusb}
          onChange={handleCusbChange}
          containerStyle={styles.valcontainer}
        />

        <SlideScale
          question={questionnaire.cusc}
          onChange={handleCuscChange}
          containerStyle={styles.valcontainer}
        />
      ...
      <View style={[styles.bottomContainer, { height: OtherContainerHeight }]}>
        <Pressable
          ...
          onBlur={handleBlur}
          onPress={async() => {
            if (acca != '' && expa != '' && expb != '' && expc != '' && intera != '' && usefa != '' && usefb != '' && trusta != '' && trustb != '' && eata != '' && useia != '' && useib != '' && useic != '' && quaa != '' && ada != '' && adb != '' && cusa != '' && cusb != '' && cusc != '' && edul != '' && workf != '' && race != '' && hper != '' && feedback != '' && extra != '')
            {
              ...
              else
              {
                ...
                {
                  setDis(true);
                  let status = await endapi(userId, acca, expa, expb, expc, intera, usefa, usefb, trusta, trustb, eata, useia, useib, useic, quaa, ada, adb, cusa, cusb, cusc, edul + '(' + edut + ')', workf, race, hper, feedback, extra);
